...
export default function ItemsList({items, setItems, page, setPagesCount, modalIsOpen, setModalIsOpen}) {
    ...
    return (
        <If condition={items.length !== 0} otherwise={<div className="empty"><p>К сожалению, по вашему запросу ничего не найдено</p></div>}>
            <button onClick={() => setModalIsOpen(value => !value)} id="open-create-item-form">Создать</button>
            <div className="items">
                {items.map(elem => <Item key={elem.id} {...elem}/>)}
            </div>
